# 📊 InsightTube – YouTube Analytics Platform

InsightTube is a powerful **YouTube Analytics Web Application** built using **Streamlit**.  
It leverages the **YouTube Data API v3** to fetch, analyze, and visualize channel and video metrics, helping users gain actionable insights.

---

## 🚀 Live Demo

🔗 **Try the App:** [Click Here](https://insighttubeo7.streamlit.app/)


---

📌 Follows a **Streamlit multi-page architecture**

---

## ✨ Key Features

### 📈 Smart Channel Analysis
- Subscriber growth tracking  
- Total views & video count  
- Engagement insights  

---

### ⚖️ Channel Comparison
- Side-by-side comparison of creators  
- Performance benchmarking  
- Visual data insights  

---

### 🔥 Trending Dashboard
- Discover trending YouTube content  
- Analyze popular categories  
- Real-time insights  

---

### 📊 Interactive Visualization
- Dynamic charts using Pandas + Plotly/Altair  
- Clean and responsive data representation  

---

### 🎨 Premium UI/UX
- Dark theme with glassmorphism design  
- Custom CSS styling  
- Smooth navigation  

---

## 🖼️ Screenshots

> 📌 Add your app screenshots below

<p align="center">
  <img src="![Home_page-0001](https://github.com/user-attachments/assets/24c4bc8c-393b-452c-bd0f-504c7ca6dde6)
" width="45%" />
  <img src="your-image-link-2" width="45%" />
</p>

<p align="center">
  <img src="your-image-link-3" width="45%" />
  <img src="your-image-link-4" width="45%" />
</p>

---

## 🛠️ Tech Stack

| Layer           | Technology              |
|----------------|------------------------|
| Frontend       | Streamlit              |
| Backend Logic  | Python                 |
| Data Handling  | Pandas                 |
| API            | YouTube Data API v3    |
| Visualization  | Plotly / Altair        |
| Styling        | Custom CSS + HTML      |

---

## ⚙️ How It Works

1. User inputs a YouTube channel  
2. API fetches real-time data  
3. Data is processed using analytics logic  
4. Insights are displayed via dashboards  

---

## 🎯 Why InsightTube?

- 📊 Real-time analytics  
- ⚡ Fast and lightweight  
- 🧠 Data-driven insights  
- 🎨 Clean UI/UX  
- 🔍 Useful for creators & analysts  

---

## 🚀 Future Enhancements

- User authentication system  
- Save & export reports  
- AI-based predictions  
- Multi-channel dashboards  
- Mobile-friendly UI  

---

## 👨‍💻 Author

**Parth Pandurang Bulbule (pbo7)**  
B.Tech IT | Developer | AI Enthusiast 🚀  

---

## ⭐ Final Thought

**InsightTube transforms raw YouTube data into meaningful insights.**
